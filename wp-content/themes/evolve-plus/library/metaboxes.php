...
class PyreThemeFrameworkMetaboxes {
	...
	function admin_script_loader() {
		...
		if (is_admin() && ($pagenow=='post-new.php' || $pagenow=='post.php')) {
	    	wp_register_script('evolve_upload', get_bloginfo('template_directory').'/library/media/js/upload.js');
	...
	public function add_meta_boxes()
	{
  	$post_types = get_post_types( array( 'public' => true ) );
		...
		$this->add_meta_box('portfolio_options', 'Portfolio Options', 'evolve_portfolio');

		$this->add_meta_box('es_options', 'Elastic Slide Options', 'theme4press_elastic');
	...
	public function upload($id, $label, $desc = '')
	{
		...
		$html .= '<div class="t4p_metabox_field">';
			...
			$html .= '<div class="field">';
			    ...
			    $html .= '<input class="evolve_upload_button" type="button" value="Browse" />';
				if($desc) {
					$html .= '<p>' . $desc . '</p>';
				}
			$html .= '</div>';
