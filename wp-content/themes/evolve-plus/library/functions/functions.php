...
function evolve_m_atts( $pairs, $atts) {
		if ($atts == null) return $pairs;
        $atts = (array)$atts;
        $out = array();
...
function evolve_check_value(){
	$oo=ReduxFrameworkInstances::get_instance( 'evl_options' );

//---------------TYPO-------------------	
	$option_name = 'evl_title_font';
	$x = $oo->get($option_name);
	$y = evolve_m_atts( array(
					'font-size' => '39px',
					'color' => '#ffffff',
					'font-family' => 'Roboto',
					'font-style' => '400',
				), $x);
	 $oo->set($option_name,$y);
//---------------TYPO END-------------------	
...
function evolve_media() {
  ...
  wp_enqueue_script( 'googlemaps', '//maps.googleapis.com/maps/api/js?v=3.exp&amp;language='.mb_substr(get_locale(), 0, 2) );        
		...
		wp_add_inline_style( 'maincss', $evolve_css_data ); 
                ...
                switch ($evolve_header_type) {
                    ...
                    case "h1":
                            ...
                            break;
                    case "h2":
                            require_once( get_template_directory() . '/css/header3.php' ); 
                            break;	
                    case "h3":
                            require_once( get_template_directory() . '/css/header4.php');
                            break;	
                    case "h4":
                            require_once( get_template_directory() . '/css/header5.php');
                            break;					
                    case "h5":
                            require_once( get_template_directory() . '/css/header6.php');
                            break;	
                }
                wp_add_inline_style( 'maincss', $evolve_css_data ); 
		// Shortcode.css
